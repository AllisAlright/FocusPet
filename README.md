...
这些不是忘了做，而是当前阶段刻意不做。MVP 先验证“宠物陪伴是否能帮助用户重新进入任务”，不是一上来做成复杂平台。
...
- 宠物是首页主角，不是装饰物。
- 首页入口服务于快速进入常用场景，但不应该抢走宠物的存在感。
- 用户和宠物聊天时，宠物可以理解意图、温和回应、必要时调用内部能力。
- 不把“拆任务”“下一步建议”等能力做成大而裸露的工具按钮。
- 对用户危险、违法、成人、医疗风险、过大目标、操纵关系结果等请求，要先安全判断，再决定是否拒绝、转向或陪伴。

## 当前 Agent 形态

当前使用的是 Pet Agent，而不是旧文档里的 Agent v2 工具中台。
